...
//      PedTree->SetBranchAddress("RunNumber",&run_number);
      ...
      TCanvas *SNRCanvas = new TCanvas("SNRCanvas", "Telescope Reference Detectors and Diamond Detector SNR",200, 400, 1200, 900);
      SNRCanvas->Divide(2,5);
      SNRCanvas->Connect("Closed()", "TApplication", &theApp, "Terminate()");
      ...
      for(int det=0; det<9; det++) {
         SNRCanvas->cd(det+1);
         if(det==8) {
            SNRHisto[det] = new TH1F(detname[det].c_str(),detname[det].c_str(),128,-0.5,127.5);
            for(int i=0; i<128; i++) 
               SNRHisto[det]->SetBinContent(i + 1, 0);
            if(verbose) {
               cout<<endl<<"Detector "<<det<<endl<<endl;
               cout<<"index\tchan\tadc\tpedmean\tpedrms\tsnr"<<endl;
            }
            for(int i=0; i<Det_NChannels[det]; i++) {
               SNRHisto[det]->SetBinContent((int)Det_Channels[det][i] + 1, float((Dia_ADC[i] - Det_PedMean[det][i]) / Det_PedWidth[det][i]));
               if(verbose) cout<<i<<"\t"<<(int)Det_Channels[det][i]<<"\t"<<Dia_ADC[i]<<"\t"<<Det_PedMean[det][i]<<"\t"<<Det_PedWidth[det][i]<<"\t"<<float((Dia_ADC[i]-Det_PedMean[det][i])/Det_PedWidth[det][i])<<endl;
            }
         }
         else {
            SNRHisto[det] = new TH1F(detname[det].c_str(),detname[det].c_str(),256,-0.5,255.5);
            for(int i=0; i<256; i++) 
               SNRHisto[det]->SetBinContent(i + 1, 0);
            if(verbose) {
               cout<<endl<<"Detector "<<det<<endl<<endl;
            ...
            for(int i=0; i<Det_NChannels[det]; i++) {
               SNRHisto[det]->SetBinContent((int)Det_Channels[det][i] + 1, float((Det_ADC[det][i] - Det_PedMean[det][i]) / Det_PedWidth[det][i]));
               if(verbose) cout<<i<<"\t"<<(int)Det_Channels[det][i]<<"\t"<<(int)Det_ADC[det][i]<<"\t"<<Det_PedMean[det][i]<<"\t"<<Det_PedWidth[det][i]<<"\t"<<float((Det_ADC[det][i]-Det_PedMean[det][i])/Det_PedWidth[det][i])<<endl;
            }
         }
         SNRHisto[det]->Draw();
         SNRHisto[det]->GetXaxis()->SetTitle("Channel");
         SNRHisto[det]->GetXaxis()->CenterTitle();
         SNRHisto[det]->GetYaxis()->SetTitle("Pedestal Width");
         SNRHisto[det]->GetYaxis()->CenterTitle();
         detname[det] += " PedWidth";
         SNRHisto[det]->SetTitle(detname[det].c_str());
         SNRCanvas->cd(det+1);
      }
      SNRCanvas->Update();
      theApp.Run();
