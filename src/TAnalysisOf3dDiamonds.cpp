...
//	cout<<cellNo<<" "<< flush;
	...
	cout<<xPredDet<<"/"<<yPredDet<<"--> "<<settings->getCellNo(xPredDet,yPredDet)<<"( "<<cellNo<<")"<<"\t--> "<<relPos.first<<"/"<<relPos.second<<endl;
	...
	if(!settings->IsWithInTheColumnRadius(relPos.first, relPos.second))
