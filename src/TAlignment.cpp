...
TAlignment::TAlignment(TSettings* settings) {
	...
	diaCalcMode = TCluster::maxValue;
	silCalcMode = TCluster::corEta;
...
void TAlignment::initialiseDetectorAlignment() {
	if (align == NULL) {
		...
		cout
				<< "TAlignment::Align::Detectoralignment did not exist, so created new DetectorAlignment"
				<< endl;
...
//	int falseClusterSizeDet=0;
	...
	align->setAlignmentTrainingTrackFraction(
			settings->getAlignment_training_track_fraction());
...
int TAlignment::Align(UInt_t nEvents, UInt_t startEvent) {
	if (verbosity) {
		cout << "\n\n\nTAlignment::Align:Starting \""
				<< histSaver->GetPlotsPath() << "\"" << endl;
		cout << "\t\t" << events.size() << "\t";
...
//	verbosity=0;
	...
	cout << "Check Detector Alignment" << endl;
	...
	cout << "ALign Detector with Plane 0" << endl;
	...
	if (verbosity)
		cout << endl;
	cout << "STart with Alignment Steps" << endl;
...
bool TAlignment::SiliconAlignmentStep(bool bPlot) {
	TPlaneProperties::enumCoordinate coordinates = (
			(nAlignmentStep < 2) ?
					TPlaneProperties::XY_COR : TPlaneProperties::Y_COR);

	cout << "\n\n\nALIGNMENT STEP:\t" << nAlignmentStep + 1 << " of "
			<< nAlignSteps << "\n\n" << endl;
	bool isAlignmentDone = true;
	if (nAlignmentStep < 2) {
		for (UInt_t plane = 0; plane < TPlaneProperties::getNSiliconPlanes();
				plane++) {
			vector<UInt_t> vecRefPlanes;
			for (UInt_t refPlane = 0;
					refPlane < TPlaneProperties::getNSiliconPlanes();
					refPlane++)
				if (refPlane != plane)
					vecRefPlanes.push_back(refPlane);
			TResidual res = CheckDetectorAlignment(TPlaneProperties::XY_COR,
					plane, vecRefPlanes, false);
			res = CheckDetectorAlignment(coordinates, plane, vecRefPlanes,
					false, res);
	...
	cout << "\nAlign Plane " << subjectPlane1 << " with Plane " << refPlane1_1
			<< " and " << refPlane1_2 << endl;
	TResidual resPlane1 = CheckDetectorAlignment(TPlaneProperties::XY_COR,
			subjectPlane1, refPlane1_1, refPlane1_2, false);
	if (verbosity)
		printf("\nnCheckAlignmentPlane1:%d,%1.2f+/-%1.2f\n",
				resPlane1.isTestResidual(), resPlane1.getXMean(),
				resPlane1.getXSigma());
	AlignDetector(coordinates, subjectPlane1, refPlane1_1, refPlane1_2,
			bPlot || plotAll, resPlane1);
	...
	UInt_t refPlane2_1 = 1, refPlane2_2 = 3;
	cout << "\nAlign Plane " << subjectPlane2 << " with Plane " << refPlane2_1
			<< " and " << refPlane2_2 << endl;
	TResidual resPlane2 = CheckDetectorAlignment(TPlaneProperties::XY_COR,
			subjectPlane2, refPlane2_1, refPlane2_2, false);
	if (verbosity)
		printf("\nCheckAlignmentPlane2:%d,%1.2f+/-%1.2f\n",
				resPlane2.isTestResidual(), resPlane2.getXMean(),
				resPlane2.getXSigma());
	AlignDetector(coordinates, subjectPlane2, refPlane2_1, refPlane2_2,
			bPlot || plotAll, resPlane2);
	xOff = align->GetLastXOffset(2);
	yOff = align->GetLastYOffset(2);
	if (xOff > 0.01 || yOff > 0.01)
		isAlignmentDone = false;

	if (true || nAlignmentStep < nAlignSteps - 2) {
		UInt_t subjectPlane3 = 3;
		UInt_t refPlane3_1 = 0, refPlane3_2 = 2;
		cout << "\nAlign Plane " << subjectPlane3 << " with Plane "
				<< refPlane3_1 << " and " << refPlane3_2 << endl;
		vector<UInt_t> vecRefPlanes;
		vecRefPlanes.push_back(refPlane3_1);
		vecRefPlanes.push_back(refPlane3_2);
//		vecRefPlanes.push_back(2);
		TResidual resPlane3 = CheckDetectorAlignment(TPlaneProperties::XY_COR,
				3, vecRefPlanes, false);
		if (verbosity)
			printf("\nnCheckAlignmentPlane3:%d,%1.2f+/-%1.2f\n",
					resPlane3.isTestResidual(), resPlane3.getXMean(),
					resPlane3.getXSigma());
		resPlane3 = AlignDetector(coordinates, 3, vecRefPlanes,
				bPlot || plotAll, resPlane3);
...
//	this->verbosity=5;
	nDiaAlignmentStep = -1;
	TResidual resDia = CheckStripDetectorAlignment(TPlaneProperties::X_COR,
			diaPlane, vecRefPlanes, false, true);
	//myTrack->setVerbosity(1);
	for (nDiaAlignmentStep = 0; nDiaAlignmentStep < nDiaAlignSteps;
			nDiaAlignmentStep++) {
		cout << "\n\n " << nDiaAlignmentStep << " of " << nDiaAlignSteps
				<< " Steps..." << endl;
//		if(nDiaAlignmentStep==0)verbosity=4;
//		else verbosity=0;
		AlignStripDetector(TPlaneProperties::X_COR, diaPlane, vecRefPlanes,
				false, resDia);
		resDia = CheckStripDetectorAlignment(TPlaneProperties::X_COR, diaPlane,
				vecRefPlanes);
	}
	verbosity = 0;
	resDia = CheckStripDetectorAlignment(TPlaneProperties::X_COR, diaPlane,
			vecRefPlanes, true, true, resDia);
//	verbosity=100;
	CheckStripDetectorAlignmentChi2(TPlaneProperties::X_COR, diaPlane,
			vecRefPlanes, true, true, settings->getAlignment_chi2());
	align->setDiamondDate();
}

TResidual TAlignment::AlignDetector(TPlaneProperties::enumCoordinate cor,
		UInt_t subjectPlane, UInt_t refPlane1, UInt_t refPlane2, bool bPlot,
		TResidual resOld) {
	vector<UInt_t> vecRefPlanes;
...
TResidual TAlignment::AlignDetector(TPlaneProperties::enumCoordinate cor,
		UInt_t subjectPlane, vector<UInt_t> vecRefPlanes, bool bPlot,
		TResidual resOld) {
	if (true) { //some outputs
		if (verbosity)
			cout << "TAlignment::AlignDetector::\taligning Plane "
					<< subjectPlane
					<< TPlaneProperties::getCoordinateString(cor) << " with "
					<< vecRefPlanes.size() << " Planes: ";
		...
		for (UInt_t i = 0; i < vecRefPlanes.size(); i++)
			if (vecRefPlanes.at(i) == subjectPlane)
				cerr
						<< "Plane "
						<< subjectPlane
						<< " is used as a reference Plane and as a subject Plane"
						<< endl;
	}

	//get Residual
	TResidual res = this->getResidual(cor, subjectPlane, vecRefPlanes, bPlot,
			resOld);
	...
	if (subjectPlane == 3) {
		cout << "Xoff: " << x_offset << "\tPhiXoff: " << phix_offset
				<< "\tYoff: " << y_offset << "\tphiY:" << phiy_offset << endl;
	}
	printf(
			"Correction Values: X: %2.6f,  PhiX: %2.6f,   Y: %2.6f,  PhiY: %2.6f\n",
	...
	if (vecRefPlanes.size() == 1) {
		...
		if (TPlaneProperties::X_COR == cor || TPlaneProperties::XY_COR == cor) {
			if (x_offset != N_INVALID)
				align->AddToXOffset(subjectPlane, x_offset);
			if (phiy_offset != N_INVALID)
				align->AddToPhiYOffset(subjectPlane, phiy_offset);
		}
		if (TPlaneProperties::Y_COR == cor || TPlaneProperties::XY_COR == cor) {
			if (y_offset != N_INVALID)
				align->AddToYOffset(subjectPlane, y_offset);
			if (phix_offset != N_INVALID)
...
TResidual TAlignment::AlignStripDetector(TPlaneProperties::enumCoordinate cor,
		UInt_t subjectPlane, vector<UInt_t> vecRefPlanes, bool bPlot,
		TResidual resOld) {
	if (verbosity) {
		cout << "TAlignment::AlignStripDetector::\taligning Plane "
				<< subjectPlane << TPlaneProperties::getCoordinateString(cor)
				<< " with " << vecRefPlanes.size() << " Planes: ";
	...
	//get residual
	TResidual res = this->getStripResidual(cor, subjectPlane, vecRefPlanes,
			false, bPlot, resOld);
	Float_t x_offset = res.getXOffset();
...
TResidual TAlignment::getResidual(TPlaneProperties::enumCoordinate cor,
		UInt_t subjectPlane, UInt_t refPlane1, UInt_t refPlane2, bool bPlot,
		TResidual resOld, TCluster::calculationMode_t mode) {
	vector<UInt_t> vecRefPlanes;
	vecRefPlanes.push_back(refPlane1);
	if (refPlane1 != refPlane2)
		vecRefPlanes.push_back(refPlane2);
...
TResidual TAlignment::getResidual(TPlaneProperties::enumCoordinate cor,
	...
	if (verbosity)
		cout << "TAlignment::getResidual of Plane " << subjectPlane
				<< TPlaneProperties::getCoordinateString(cor) << " with "
				<< refPlaneString.str() << ", plotting: " << bPlot << "  with "
				<< alignmentPercentage << "\t" << resOld.isTestResidual()
				<< endl;
	clearEventVectors();
	...
	for (UInt_t nEvent = 0; nEvent < events.size(); nEvent++) {
		...
		xPositionObserved = myTrack->getPosition(TPlaneProperties::X_COR,
				subjectPlane, mode);
		yPositionObserved = myTrack->getPosition(TPlaneProperties::Y_COR,
				subjectPlane, mode);
		...
		if (verbosity > 3)
			cout << "Measured: " << myTrack->getXMeasured(subjectPlane, mode)
					<< "/" << myTrack->getYMeasured(subjectPlane, mode) << endl;
		if (verbosity > 3)
			cout << "Observed: " << xPositionObserved << " / "
					<< yPositionObserved << endl;
		if (verbosity > 3)
			cout << "Predicted: " << predictedPostion->getPositionX() << " / "
					<< predictedPostion->getPositionY() << endl;
		if (verbosity > 3)
			cout << "Delta:    " << xDelta << " / " << yPositionObserved
					<< endl;
		if (verbosity > 3)
			cout << "ResTest:  " << resxtest << " / " << resytest << "\n\n"
					<< endl;
	...
	if (verbosity > 2)
		cout << vecXDelta.size() << " " << vecYDelta.size() << " "
				<< vecXPred.size() << " " << vecYPred.size() << endl;
 ...
 * @return
 */
TResidual TAlignment::getStripResidual(TPlaneProperties::enumCoordinate cor,
		UInt_t subjectPlane, vector<UInt_t> vecRefPlanes, bool bAlign,
		bool bPlot, TResidual resOld, TCluster::calculationMode_t mode) {
	...
	for (UInt_t i = 0; i < vecRefPlanes.size(); i++)
		if (i == 0)
			...
			refPlaneString << "_and_" << vecRefPlanes.at(i);
	if (verbosity)
		cout << "TAlignment::getStripResidual of Plane " << subjectPlane
				<< TPlaneProperties::getCoordinateString(cor) << " with "
				<< refPlaneString.str() << ", plotting: " << bPlot << "  with "
				<< alignmentPercentage << "\t" << resOld.isTestResidual()
				<< endl;
	...
	for (UInt_t nEvent = 0; nEvent < events.size(); nEvent++) {
		...
		myTrack->setEvent(&events.at(nEvent));
		predictedPostion = myTrack->predictPosition(subjectPlane, vecRefPlanes,
				TCluster::corEta, verbosity > 1);
		xPositionObserved = myTrack->getStripXPosition(subjectPlane,
				predictedPostion->getPositionY(), TCluster::maxValue);
		yPositionObserved = myTrack->getPosition(TPlaneProperties::Y_COR,
				subjectPlane, TCluster::maxValue);
		...
		if (verbosity > 3)
			cout << "Measured: " << myTrack->getXMeasured(subjectPlane) << "/"
					<< myTrack->getYMeasured(subjectPlane) << endl;
		if (verbosity > 3)
			cout << "Observed: " << xPositionObserved << " / "
					<< yPositionObserved << endl;
		if (verbosity > 3)
			cout << "Predicted: " << predictedPostion->getPositionX() << " / "
					<< predictedPostion->getPositionY() << endl;
		if (verbosity > 3)
			cout << "Delta:    " << deltaX << " / " << yPositionObserved
					<< endl;
		if (verbosity > 3)
			cout << "ResTest:  " << resxtest << " / " << resytest << "\n\n"
					<< endl;
...
TResidual TAlignment::getStripResidualChi2(TPlaneProperties::enumCoordinate cor,
		UInt_t subjectPlane, vector<UInt_t> vecRefPlanes, bool bAlign,
		bool bPlot, Float_t maxChi2, TCluster::calculationMode_t mode) {
	...
	for (UInt_t nEvent = 0; nEvent < events.size(); nEvent++) {
		...
		predictedPosition = myTrack->predictPosition(subjectPlane, vecRefPlanes,
				TCluster::corEta, nEvent < 1);
		chi2x = predictedPosition->getChi2X();
		chi2y = predictedPosition->getChi2Y();
		xPositionObserved = myTrack->getStripXPosition(subjectPlane,
				predictedPosition->getPositionY(), TCluster::maxValue);
		yPositionObserved = myTrack->getPosition(TPlaneProperties::Y_COR,
				subjectPlane, TCluster::maxValue);
		...
		xMeasured = myTrack->getMeasured(TPlaneProperties::X_COR, subjectPlane,
				TCluster::maxValue);
		yMeasured = myTrack->getMeasured(TPlaneProperties::Y_COR, subjectPlane,
				TCluster::maxValue);
		resxtest = TMath::Abs(deltaX - resOld.getXMean()) / resOld.getXSigma();
		resytest = TMath::Abs(deltaY - resOld.getYMean()) / resOld.getYSigma();
		if (verbosity > 3)
		...
		if (verbosity > 3)
			cout << "Observed: " << xPositionObserved << " / "
					<< yPositionObserved << endl;
		if (verbosity > 3)
			cout << "Predicted: " << predictedPosition->getPositionX() << " / "
					<< predictedPosition->getPositionY() << endl;
		if (verbosity > 3)
			cout << "Delta:    " << deltaX << " / " << yPositionObserved
					<< endl;
		if (verbosity > 3)
			cout << "Chi2:     " << chi2x << " / " << chi2y << endl;
		if (verbosity > 3)
			cout << "ResTest:  " << resxtest << " / " << resytest << endl;

		if (chi2x < settings->getAlignment_chi2()
				&& chi2y < settings->getAlignment_chi2()) {
			if (verbosity > 3)
				cout << "Take event for Residual" << endl;
			vecXObs.push_back(xPositionObserved);
			vecYObs.push_back(yPositionObserved);
			vecXDelta.push_back(deltaX);
			vecYDelta.push_back(deltaY);
...
//			if(verbosity>3)	cout<< deltaX<<" "<<deltaY<<endl;
	...
	if (verbosity)
		cout << vecXDelta.size() << " " << vecYDelta.size() << " "
				<< vecXPred.size() << " " << vecYPred.size() << " "
				<< vecXObs.size() << " " << vecYObs.size() << endl;
	if (verbosity)
		cout << "used " << vecXDelta.size() << " Events of " << events.size()
				<< " with a Chi2 < " << settings->getAlignment_chi2() << endl;
	//first estimate residuals widths
	TResidual res;
	res.setResKeepFactor(res_keep_factor);
	res.calculateResidual(cor, &vecXPred, &vecXDelta, &vecYPred, &vecYDelta);
 ...
 */
TResidual TAlignment::CheckDetectorAlignment(
		TPlaneProperties::enumCoordinate cor, UInt_t subjectPlane,
		UInt_t refPlane1, UInt_t refPlane2, bool bPlot, TResidual resOld) {
	vector<UInt_t> vecRefPlanes;
	vecRefPlanes.push_back(refPlane1);
	vecRefPlanes.push_back(refPlane2);
	return CheckDetectorAlignment(cor, subjectPlane, vecRefPlanes, bPlot,
			resOld);
}

TResidual TAlignment::CheckDetectorAlignment(
		TPlaneProperties::enumCoordinate cor, UInt_t subjectPlane,
		vector<UInt_t> vecRefPlanes, bool bPlot, TResidual resOld) {
	if (verbosity)
		cout << "\n\nTAlignment::checkDetectorAlignment\n\t check "
				<< TPlaneProperties::getCoordinateString(cor)
				<< " coordinate of Plane " << subjectPlane << " with "
				<< vecRefPlanes.size() << " Planes" << endl;
...
TResidual TAlignment::CheckStripDetectorAlignment(
		TPlaneProperties::enumCoordinate cor, UInt_t subjectPlane,
		vector<UInt_t> vecRefPlanes, bool bAlign, bool bPlot,
		TResidual resOld) {
	int verb = verbosity;
	verbosity = 0;
	TResidual res = getStripResidual(cor, subjectPlane, vecRefPlanes, false,
			false, resOld);
	if (verbosity)
		cout << endl;
	res.SetTestResidual(false);
...
TResidual TAlignment::CheckStripDetectorAlignmentChi2(
		TPlaneProperties::enumCoordinate cor, UInt_t subjectPlane,
		vector<UInt_t> vecRefPlanes, bool bAlign, bool bPlot, Float_t maxChi2) {
	int verb = verbosity;
	verbosity = 1;
...
//	getStripResidual(cor,subjectPlane,vecRefPlanes,false,false,maxChi2);
	if (verbosity)
		cout << endl;
	TResidual res = getStripResidualChi2(cor, subjectPlane, vecRefPlanes,
			bAlign, bPlot, maxChi2);
	if (verbosity)
		res.Print();
	verbosity = verb;
	return res;
...
void TAlignment::saveAlignment() {
	...
	cout << "TAlignment:saveAlignment(): path: \"" << sys->pwd()
			<< "\", file Name:\"" << fileName.str() << "\"" << endl;
	alignmentFile->cd();
...
//	myTrack->setVerbosity(4);
	...
	for (UInt_t nEvent = 0; nEvent < events.size(); nEvent++) {
		...
		chi2X = predictedPosition->getChi2X();
		chi2Y = predictedPosition->getChi2Y();
		if (predictedPosition->getChi2X() < maxChi2
				&& predictedPosition->getChi2Y() < maxChi2) {
			for (subjectPlane = 0; subjectPlane < 4; subjectPlane++) {
				if (subjectPlane != 0) {
					predictedPosition->Delete();
					predictedPosition = myTrack->predictPosition(subjectPlane,
							vecRefPlanes, TCluster::corEta, false);
				}
...
//	myTrack->setVerbosity(oldVerbosity);
	...
	TH1F histoChi2X = histSaver->CreateDistributionHisto(histName.str(),
			vecXChi2, 4096, HistogrammSaver::positiveSigma);
	...
	TH1F histoChi2Y = histSaver->CreateDistributionHisto(histName.str(),
			vecYChi2, 4096, HistogrammSaver::positiveSigma);
	...
	TH2F histo = histSaver->CreateScatterHisto(histName.str(), vecSumDeltaX,
			vecXChi2);
	...
	TH2F histo1 = histSaver->CreateScatterHisto(histName.str(), vecSumDeltaY,
			vecYChi2);
	histName << "_graph";
	TGraph graph1 = histSaver->CreateDipendencyGraph(histName.str(),
			vecSumDeltaY, vecYChi2);
	graph1.Draw("AP");
 ...
 */
TResidual TAlignment::calculateResidualWithChi2(
		TPlaneProperties::enumCoordinate cor, UInt_t subjectPlane,
		vector<UInt_t> vecRefPlanes, Float_t maxChi2, bool bAlign, bool bPlot) {
	if (verbosity)
	...
	//	UInt_t oldVerbosity=myTrack->getVerbosity();
	//	myTrack->setVerbosity(4);
	Float_t chi2x, chi2y, xPositionObserved, yPositionObserved, deltaX, deltaY;
	; //_
	for (UInt_t nEvent = 0; nEvent < events.size(); nEvent++) {
		TRawEventSaver::showStatusBar(nEvent, events.size());
		myTrack->setEvent(&events.at(nEvent));
		...
		predictedPosition = myTrack->predictPosition(subjectPlane, vecRefPlanes,
				TCluster::corEta);
		if (verbosity > 3)
			predictedPosition->Print();
		if (verbosity > 3)
			cout << xPositionObserved << " / " << yPositionObserved << endl;
		deltaX = xPositionObserved - predictedPosition->getPositionX(); //X_OBS-X_Pred
		deltaY = yPositionObserved - predictedPosition->getPositionY(); //Y_OBS-Y_Pred
	...
	cout << "use " << vecXDelta.size() << " of " << events.size()
			<< " Events ( "
			<< (Float_t) vecXDelta.size() / (Float_t) events.size() * 100.
			<< " %)" << endl;
...
void TAlignment::getFinalSiliconAlignmentResuluts() {
	...
	cout << "get Final Silicon Resolution with a  maximum chi2 of " << maxChi2
			<< endl;
	//set Resolutions
...
void TAlignment::setSiliconDetectorResolution(Float_t maxChi2) {
	...
	for (UInt_t plane = 0; plane < 4; plane++) {
		...
		for (UInt_t i = 0; i < 4; i++)
			if (i != plane)
				vecRefPlanes.push_back(i);
		TResidual res = CheckDetectorAlignment(TPlaneProperties::XY_COR, plane,
				vecRefPlanes, false);
		res = CheckDetectorAlignment(TPlaneProperties::XY_COR, plane,
				vecRefPlanes, false, res);
		align->setXResolution(res.getXSigma(), plane);
		align->setYResolution(res.getYSigma(), plane);
	}
	...
	for (UInt_t plane = 0; plane < 4; plane++) {
		...
		for (UInt_t i = 0; i < 4; i++)
			if (i != plane)
				vecRefPlanes.push_back(i);
		TResidual res = calculateResidualWithChi2(TPlaneProperties::XY_COR,
				plane, vecRefPlanes, maxChi2, true, true);
	}
}

void TAlignment::CreatePlots(TPlaneProperties::enumCoordinate cor,
		UInt_t subjectPlane, string refPlaneString, bool bPlot,
		bool bUpdateAlignment, bool bChi2) {
	if (bPlot)
		if (verbosity)
			cout << vecXDelta.size() << " " << vecYDelta.size() << " "
					<< vecXPred.size() << " " << vecYPred.size() << " "
					<< vecXObs.size() << " " << vecYObs.size() << endl;
	if (!bPlot && !bUpdateAlignment)
		return;
	...
	if (subjectPlane == 4) {
		...
		if (nAlignmentStep == -1)
			...
			preName << nAlignmentStep << "Step";
	}
	stringstream histName;
	cout << "\nCreatePlots with " << preName.str() << " " << nAlignmentStep
			<< flush;
	if (bUpdateAlignment)
	...
	if (cor == TPlaneProperties::XY_COR || cor == TPlaneProperties::X_COR) { //DistributionPlot DeltaX
		...
		TH1F* histo = (TH1F*) histSaver->CreateDistributionHisto(histName.str(),
				vecXDelta, 512, HistogrammSaver::threeSigma).Clone();
		...
		if (bUpdateAlignment) {
			...
			cout << "set Resolution via Gaus-Fit: " << xRes << " with "
					<< vecXDelta.size() << " Events" << endl;
	...
	if (bPlot
			&& (cor == TPlaneProperties::XY_COR
					|| cor == TPlaneProperties::X_COR)) { //DistributionPlot DeltaX vs Ypred
		histName.str("");
		histName.clear();
		histName << preName.str();
		...
		histName << "_-_Plane_" << subjectPlane << "_with_" << refPlaneString;
		; //<<"_with"<<refPlane1<<"_and_"<<refPlane2;
		TH2F histo = histSaver->CreateDipendencyHisto(histName.str(), vecYPred,
				vecXDelta, 256);
		histo.Draw("goff");
		...
		histSaver->SaveHistogram(&histo);
		histName << "_graph";
		TGraph graph = histSaver->CreateDipendencyGraph(histName.str(),
				vecXDelta, vecYPred);
		graph.Draw("APL");
	...
	if (bPlot
			...
			&& subjectPlane == TPlaneProperties::getDiamondPlane()) { //DistributionPlot DeltaX vs Xpred
		histName.str("");
		histName.clear();
		histName << preName.str();
		...
		histName << "_-_Plane_" << subjectPlane << "_with_" << refPlaneString;
		; //<<"_with"<<refPlane1<<"_and_"<<refPlane2;
		TH2F histo = histSaver->CreateDipendencyHisto(histName.str(), vecXPred,
				vecXDelta, 256);
		histo.Draw("goff");
	...
	if (bPlot
		...
		histName << "_-_Plane_" << subjectPlane << "_with_" << refPlaneString;
		; //<<"_with"<<refPlane1<<"_and_"<<refPlane2;
		TH2F histo = histSaver->CreateDipendencyHisto(histName.str(),
				vecXMeasured, vecXDelta, 256);
		histo.Draw("goff");
	...
	if (subjectPlane < 4
			&& (cor == TPlaneProperties::XY_COR
					|| cor == TPlaneProperties::Y_COR)) { //DistributionPlot DeltaY
		histName.str("");
		histName.clear();
		histName << preName.str();
		...
		TH1F* histo = (TH1F*) histSaver->CreateDistributionHisto(histName.str(),
				vecYDelta, 512, HistogrammSaver::threeSigma).Clone();
	...
	if (bPlot && subjectPlane < 4
			&& (cor == TPlaneProperties::XY_COR
					|| cor == TPlaneProperties::Y_COR)) { //ScatterPlot DeltaY vs Xpred
		histName.str("");
		histName.clear();
		histName << preName.str();
	...
	if (bPlot && subjectPlane < 4
			&& (cor == TPlaneProperties::XY_COR
					|| cor == TPlaneProperties::Y_COR)) { //ScatterHisto XObs vs YObs
		histName.str("");
		histName.clear();
		histName << preName.str();
		...
		histName << "_-_Plane_" << subjectPlane << "_with_" << refPlaneString;
		; //<<"_with"<<refPlane1<<"_and_"<<refPlane2;
		TH2F histo = histSaver->CreateScatterHisto(histName.str(), vecXObs,
				vecYObs);
	...
	if (bPlot && nAlignmentStep > -1
			&& (cor == TPlaneProperties::XY_COR
					|| cor == TPlaneProperties::Y_COR)) { //ScatterHisto DeltaX vs Chi2X
		histName.str("");
		histName.clear();
		histName << preName.str();
		...
		histName << "_-_Plane_" << subjectPlane << "_with_" << refPlaneString;
		; //<<"_with"<<refPlane1<<"_and_"<<refPlane2;
		TH2F histo = histSaver->CreateScatterHisto(histName.str(), vecXDelta,
				vecXChi2, 256);
		histo.Draw("goff");
	...
	if (bPlot && nAlignmentStep > -1 && subjectPlane < 4
			&& (cor == TPlaneProperties::XY_COR
					|| cor == TPlaneProperties::Y_COR)) { //ScatterHisto DeltaY vs Chi2Y
		histName.str("");
		histName << preName.str();
		histName << "_ScatterPlot_DeltaX_vs_Chi2X";
		histName << "_-_Plane_" << subjectPlane << "_with_" << refPlaneString;
		; //<<"_with"<<refPlane1<<"_and_"<<refPlane2;
		TH2F histo = histSaver->CreateScatterHisto(histName.str(), vecYDelta,
				vecYChi2, 256);
		histo.Draw("goff");
		histo.GetYaxis()->SetTitle("Delta Y");
	...
	if (bPlot && subjectPlane == 4
			&& (cor == TPlaneProperties::XY_COR
					|| cor == TPlaneProperties::X_COR)) { //predX vs deltaX
		histName.str("");
		histName << preName.str();
		histName << "_ScatterPlot_XPred_vs_DeltaX";
		histName << "_-_Plane_" << subjectPlane << "_with_" << refPlaneString;
	...
	if (bPlot && subjectPlane == 4
			&& (cor == TPlaneProperties::XY_COR
					|| cor == TPlaneProperties::X_COR)) { //DeltaX vs ClusterSize
		histName.str("");
		histName << preName.str();
...
void TAlignment::DoEtaCorrectionSilicon(UInt_t correctionStep) {
	cout << "****************************************" << endl;
	cout << "Do Eta correction for all silicon planes " << correctionStep
			<< endl;
	cout << "****************************************" << endl;
	vector<TH1F*> histoStripDistribution, histoStripDistributionFlattned;
	vector<TH1F *> vecHEta;
	stringstream fileName;
	fileName << "etaIntegral_Step" << correctionStep << "."
			<< settings->getRunNumber() << ".root";
	...
	for (UInt_t det = 0; det < TPlaneProperties::getNSiliconDetectors();
		...
		histoTitle << "hPredictedStripPosition" << "_step" << correctionStep
				<< "_" << TADCEventReader::getStringForDetector(det);
		histoStripDistribution.push_back(
				new TH1F(histoTitle.str().c_str(), histoTitle.str().c_str(),
						128, -0.501, 0.501));
		histoTitle << "_flattend";
		histoStripDistributionFlattned.push_back(
				new TH1F(histoTitle.str().c_str(), histoTitle.str().c_str(),
						128, -0.501, 0.501));
		histoTitle.str("");
		histoTitle.clear();
		histoTitle << "hCorrectedEtaDistribution" << "_step" << correctionStep
				<< "_" << TADCEventReader::getStringForDetector(det);
		vecHEta.push_back(
				new TH1F(histoTitle.str().c_str(), histoTitle.str().c_str(),
						128, 0, 1));
	...
	for (nEvent = 0; nEvent < this->events.size(); nEvent++) {
		...
		for (UInt_t subjectPlane = 0;
				subjectPlane < TPlaneProperties::getNSiliconPlanes();
				subjectPlane++) {
...
//				continue;
			...
			for (UInt_t refPlane = 0;
					refPlane < TPlaneProperties::getNSiliconPlanes();
					refPlane++)
				if (subjectPlane != refPlane)
					refPlanes.push_back(refPlane);
			TPositionPrediction* pred = myTrack->predictPosition(subjectPlane,
					refPlanes, TCluster::corEta, false);
			Float_t predictedStripPositionX = myTrack->getPositionInDetSystem(
					subjectPlane * 2, pred->getPositionX(),
					pred->getPositionY());
			Float_t predictedStripPositionY = myTrack->getPositionInDetSystem(
					subjectPlane * 2 + 1, pred->getPositionX(),
					pred->getPositionY());
...
//			histoStripDistribution.at(subjectPlane*2)->Fill(deltaY);
			...
			Int_t binX =
					histoStripDistributionFlattned.at(subjectPlane)->FindBin(
							deltaX);
			Int_t binY =
					histoStripDistributionFlattned.at(subjectPlane)->FindBin(
							deltaY);
			if (histoStripDistributionFlattned.at(subjectPlane * 2)->GetBinContent(
					binX) < vecMinEntries.at(subjectPlane * 2)) {
				vecEventNo[subjectPlane * 2].push_back(nEvent);
				histoStripDistributionFlattned.at(subjectPlane * 2)->Fill(
						deltaX);
			}
			if (histoStripDistributionFlattned.at(subjectPlane * 2 + 1)->GetBinContent(
					binY) < vecMinEntries.at(subjectPlane * 2 + 1)) {
				vecEventNo[subjectPlane * 2 + 1].push_back(nEvent);
				histoStripDistributionFlattned.at(subjectPlane * 2 + 1)->Fill(
						deltaY);
			}
		}
	}
	for (UInt_t det = 0; det < TPlaneProperties::getNSiliconDetectors();
			det++) {
		cout << "save histogram: " << det << "  "
				<< histoStripDistributionFlattned.at(det)->GetTitle() << "  "
				<< histoStripDistribution.at(det)->GetTitle() << endl;
	...
	for (UInt_t det = 0; det < TPlaneProperties::getNSiliconDetectors();
		...
		stringstream histName;
		histName << "hEtaIntegral" << "_step" << correctionStep << "_"
				<< TADCEventReader::getStringForDetector(det);
		;
		UInt_t nBins = vecHEta.at(det)->GetNbinsX();
		TH1F *histo = new TH1F(histName.str().c_str(), histName.str().c_str(),
				nBins, 0, 1);
		...
		cout << "save " << vecHEta.at(det)->GetTitle() << " "
				<< vecHEta.at(det)->GetEntries() << endl;
		histSaver->SaveHistogram(vecHEta.at(det));
		correctedEtaFile->Add(vecHEta.at(det));
		cout << "save " << histo->GetTitle() << " " << histo->GetEntries()
				<< endl;
		histSaver->SaveHistogram(histo);
...
void TAlignment::PrintEvents(UInt_t maxEvent, UInt_t startEvent) {
	if (maxEvent == 0)
		maxEvent = eventReader->GetEntries();
	if (maxEvent > eventReader->GetEntries())
		maxEvent = eventReader->GetEntries();
...
void TAlignment::clearEventVectors() {
	vecXPred.clear();
	vecYPred.clear();
	vecXObs.clear();
	vecYObs.clear();
