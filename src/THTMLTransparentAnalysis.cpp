...
void THTMLTransparentAnalysis::createPulseHeightPlots(vector<pair <UInt_t,Float_t> > meanPulseHeigths) {
	// TODO: change this:
	subjectDetector = 8;
...
//	if(meanPulseHeigths.size()<TPlaneProperties::getNDetectors()) meanPulseHeigths.resize(TPlaneProperties::getNDetectors());
	...
	for (UInt_t clusterSize = 0; clusterSize < meanPulseHeigths.size(); clusterSize++) {
		vecTable.at(0).push_back(floatToString(meanPulseHeigths.at(clusterSize).first));
		vecTable.at(1).push_back(floatToString(meanPulseHeigths.at(clusterSize).second));
	}
...
void THTMLTransparentAnalysis::createResolutionPlots() {
